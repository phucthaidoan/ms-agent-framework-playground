...
namespace Samples.Labs.Recipe.V1_HostLoop;

// V1: Host-Orchestrated Refinement Loop
// New concept: the iterative critic→planner feedback loop runs in host code via sequential
// RunAsync<T> calls — one call to generate, one call to critique, one call to refine.
// Compare with the base sample where AsAIFunction() hides this loop inside the LLM.
// Here every iteration is explicit: you can see what the critic said and what the planner fixed.

public static class MealPlannerV1
{
    ...
    {
        Output.Title("Meal Planner V1 — Host-Orchestrated Refinement Loop");
        Output.Separator();
        ...
        // --- Step 2: Build two independent agents ---
        // Neither agent knows about the other. The host drives all orchestration.
        ...
        // --- Step 3: Refinement loop — fully visible in host code ---
        // Session keeps conversation history so the planner can build on prior output
        // across generate → refine iterations, preventing JSON truncation on large plans.
