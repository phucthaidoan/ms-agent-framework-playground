...
namespace Samples.Labs.Recipe.V2_ChatLoop;

// V2: Multi-Turn Chat Loop
// New concept: AgentSession shared across multiple RunAsync calls — the advisor agent
// remembers the entire conversation history, so follow-up questions build on prior answers.
// Compare with V1 where every RunAsync call is independent (no memory between calls).
// Here, CreateSessionAsync() creates the session once; every call passes the same session.

public static class MealPlannerV2
{
    ...
    {
        Output.Title("Meal Planner V2 — Multi-Turn Chat Loop");
        Output.Separator();
        ...
        // --- Step 2: Build planner + critic agents (same as V1) ---
        ...
        // --- Step 3: Refinement loop (same as V1) ---
        AgentSession plannerSession = await plannerAgent.CreateSessionAsync();

        Output.Title("Step 1: Generating initial meal plan...");
        ...
        // --- Step 5: Print final plan ---
        Output.Separator();
        Output.Title("Step 3: Final Meal Plan");
        PrintMealPlan(plan);

        // --- Step 6: Multi-turn chat with meal advisor ---
        // KEY CONCEPT: CreateSessionAsync() — session preserves conversation history across turns
        Output.Separator();
        Output.Title("Step 4: Meal Plan Advisor (type 'exit' to quit)");
        ...
        // KEY CONCEPT: one session created once — passed to every RunAsync call below
        AgentSession advisorSession = await mealAdvisorAgent.CreateSessionAsync();

        // Pre-seed: load the approved plan as context before the user asks anything
        string planContext =
        ...
        // KEY CONCEPT: same advisorSession on every turn — history preserved automatically
        while (true)
        {
            Console.Write("> ");
